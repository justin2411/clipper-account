...
2. [ ] Cloudflare: API-Token + Account-ID in `.env` → `./scripts/cf_bootstrap.sh` (D1, R2, Migration `worker/migrations/0001_schema.sql`,
       Deploy, Worker-Secrets: CLIPFORGE_API_KEY, BLOTATO_API_KEY, TELEGRAM_*, GMAIL_*, GITHUB_TOKEN, ACCOUNTS_JSON).
       Danach `worker/wrangler.toml` (database_id) committen. Cron läuft über `[triggers]` in `wrangler.toml`.
