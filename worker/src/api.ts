...
export async function handleRequest(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (["review", "settings", "tasks", "report", "ab", "log", "onboarding", "suggest", "inbox", "chat", "calendar", "payouts", "library", "anomalies", "reconcile", "pacer", "catalog"].includes(seg[0]) || (seg[0] === "accounts" && seg[2] === "resume")) {
    ...
    try {
      ...
      // Taktgeber (Beobachtungsmodus): GET /pacer liefert den letzten Lauf, ?fresh=1 rechnet neu. Er ändert nie etwas.
      if (seg[0] === "pacer" && !seg[1] && req.method === "GET")
        return J(url.searchParams.get("fresh") === "1" ? await buildPacer(env, ws) : (await lastPacer(env, ws)) ?? await buildPacer(env, ws));
      if (seg[0] === "pacer" && seg[1] === "run" && req.method === "POST") return J(await runPacer(env, ws));
      ...
      if (seg[0] === "settings" && !seg[1] && req.method === "PUT") {
        ...
        const { errors, warnings } = await validateAll(env, next, ws, cur);
        if (errors.length) return J({ ok: false, errors }, 400);
        const diff = diffSettings(env, cur, next);
        if (!body.confirm || body.preview || url.searchParams.get("preview") === "1") return J({ ok: false, preview: true, diff, warnings, hint: "Mit confirm:true senden, um zu schreiben" });   // Stufe 3: Diff zuerst, Bestätigung nötig
      ...
      if (seg[0] === "settings" && seg[1] === "reset" && req.method === "POST") {       // Stufe 3: auf Version oder Standard zurücksetzen (Diff zuerst)
        ...
        if (errors.length) return J({ ok: false, errors }, 400);
        const diff = diffSettings(env, cur, next);
        if (!body.confirm) return J({ ok: false, preview: true, diff, warnings, target: body.version ? `Version ${body.version}` : "Standard" });
        if (!diff.length) return J({ ok: true, unchanged: true, diff });
        const r = await putSettings(env, next, ws, diff);
